import { supabase, boot, money, esc, toast } from "./supabase.js";
...
document.getElementById("search").oninput=e=>{query=e.target.value.trim();load()};document.querySelectorAll(".tab").forEach(b=>b.onclick=()=>{filter=b.dataset.filter;document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));b.classList.add("active");load()});load();
